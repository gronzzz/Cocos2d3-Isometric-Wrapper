...
- (instancetype)initWithTilePosition:(CGPoint)tilePosition {
    self = [self init];
    if (self) {
        self.position = [[TileMap shared] worldFromTile:tilePosition];
    }
...
- (void)moveSpritePosition:(CGPoint)targetPosition sender:(id)sender {
	...
	float angle = [[TileMap shared] getAngleBetweenPoints:spritePos pt2:targetPosition];
...
//	[self runAction:[CCActionRepeatForever actionWithAction:[CCActionAnimate actionWithAnimation:self.animation]]];
	spriteRunAction = [CCActionSequence actions:
                       [CCActionMoveBy actionWithDuration:realMoveDuration position:diff],
                       [CCActionCallFunc actionWithTarget:self selector:@selector(spriteMoveFinished)],
                       nil];
...
//	CGPoint tilePos = [[CoordinateFunctions shared] tilePosFromLocation:self.position];
...
{
	float lowestZ = -([TileMap shared].map.mapSize.width + [TileMap shared].map.mapSize.height);
...
- (BOOL)checkIfPointIsInSight:(CGPoint)targetPos enemySprite:(GameSprite *)enemy {
	...
	for (int i = 0; i < [points count]; i++) {
		...
		CGPoint thisTile = [[TileMap shared] tileFromWorld:thisPoint];
		if ([[TileMap shared] isTilePosBlocked:thisTile]) {
			lineOfSight = NO;
		}
	}
	
	 if (lineOfSight) {
         CCDrawNode *line = [[CCDrawNode alloc] init];
         [line drawSegmentFrom:ccp(targetPos.x, targetPos.y) to:ccp(attackerPos.x, attackerPos.y)
                        radius:1.0 color:[CCColor colorWithRed:255 green:0 blue:255]];
     }
